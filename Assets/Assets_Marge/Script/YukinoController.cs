...
public class YukinoController : MonoBehaviour
{
    ...
    void MoveControroller()//移動操作スクリプト
    {
        if (Ground == true || Grider == true)//移動制限
        {
            ...
            if (DushCount > 0.0f && Move.magnitude > 0.0f)
            {
                ...
                childObject[index].transform.localScale = new Vector3(1.0f, 1.0f, 1.0f);
            }
            ...
            else
            {
                ...
                index = 0;
                childObject[index].SetActive(true);
                childObject[index].transform.localScale = new Vector3(2.0f, 2.0f, 2.0f);
            }
    ...
    public void HipDrop(InputAction.CallbackContext context)
    {
        if (context.started)
        {
            if (Ground == false)
            {
                rb.AddForce(new Vector3(0.0f, hipdrop, 0.0f), ForceMode.Impulse);
                Debug.Log("hipdrop");
                childObject[index].SetActive(false);
                index = 1;
                childObject[index].SetActive(true);
